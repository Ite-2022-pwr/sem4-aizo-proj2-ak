  ...
  for _, density := range Densities {
    ...
    for _, verticiesNum := range VerticiesNumbers {
      ...
      for i, graphAnal := range graphAnals {
        log.Println(utils.YellowColor(fmt.Sprintf("[+] Bellman-Ford %v wierzchołków %v%% gęstości - pomiar %v/%v", verticiesNum, density, i+1, NumberOfGraphs)))
        listTime, matrixTime := graphAnal.BellmanFord(0, verticiesNum - 1)
    ...
    utils.SaveCSV(filepath.Join(OutputDirectory, fmt.Sprintf("bellmanford_list_density_%v.csv", density)), outputList)
    utils.SaveCSV(filepath.Join(OutputDirectory, fmt.Sprintf("bellmanford_matrix_density_%v.csv", density)), outputMatrix)
  }
}
