...
func (ga *GraphAnalyzer) Dijstkra(startVertex, finishVertex int) (listTime, matrixTime float64) {
  ...
  prompt := utils.BlueColor(fmt.Sprintf("Znajdowanie najkrótszej ścieżki algorytmem Dijstkry dla listy sąsiedztwa od wierzchołka %v do %v", startVertex, finishVertex))
  log.Println("[*] Rozpoczynanie:", prompt)
  start := time.Now()

  distances, parents, err := algo.Dijkstra(ga.List, startVertex)
  ...
  prompt = utils.BlueColor(fmt.Sprintf("Znajdowanie najkrótszej ścieżki algorytmem Dijstkry dla macierzy incydencji od wierzchołka %v do %v", startVertex, finishVertex))
  log.Println("[*] Rozpoczynanie:", prompt)
  start = time.Now()

  distances, parents, err = algo.Dijkstra(ga.Matrix, startVertex)
